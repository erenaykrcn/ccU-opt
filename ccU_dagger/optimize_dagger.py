import numpy as np
from ansatz_dagger import ansatz_dagger, ansatz_grad_vector_dagger
from hessian_dagger import ansatz_hessian_matrix_dagger
from utils_dagger import polar_decomp, real_to_antisymm
from rqcopt.trust_region import riemannian_trust_region_optimize



def err_dagger(glist, U, L, perms, cU):
    return -np.trace(cU.conj().T @ ansatz_dagger(glist, U, L, perms)).real


def optimize_circuit_dagger(L: int, U, cU, Glist_start, perms, **kwargs):
    """
    Optimize the quantum gates using a trust-region method.
    """

    # target function
    f = lambda glist: err_dagger(glist, U, L, perms, cU)
    gradfunc = lambda glist: -ansatz_grad_vector_dagger(glist, cU, U, L, perms)
    hessfunc = lambda glist: -ansatz_hessian_matrix_dagger(glist, cU, U, L, perms)
    
    # quantify error by spectral norm
    errfunc = lambda glist: np.linalg.norm(ansatz_dagger(glist, U, L, perms) - cU, ord=2)
